...
// ------------------ configuration showroomAreaControl ----------------------

const Triangle = 'Triangle';
const Rectangle = 'Rectangle';
const Circle = 'Circle';
const Polygon = 'Polygon'; // not used here so far, but in other parts
...
// ----------------- end of showroomAreaControl configuration -------------------------------
...
showVelocity.style.top = '25px'; 
showVelocity.style.left = paneRadius.toString( ) / 4 + 'px';
...
navs[ cNo ].addEventListener( 'dblclick', dblclick );

navs[ cNo ].style.overflow = 'hidden';
navs[ cNo ].style.position = 'absolute';
navs[ cNo ].style.background = AreaControlIni[ cNo ][ 1 ].toString();
navs[ cNo ].style.opacity = 'cNo,5';
navs[ cNo ].style.border = '2px solid ' + AreaControlIni[ cNo ][ 2 ].toString();
navs[ cNo ].style.cursor = 'move';
	...
	for (  let i = 1; i < cNo; i ++ ){
		...
		if ( navAction[ i ] && i < navActionCommands.length ) {
			...
			if ( i > 4 && !block[ i ] ) {
				...
				if ( !withinWalkableAreas( eyes, walkableAreas ) ) {  //  define your walkableAreas array
...
function withinWalkableAreas( vE, wA ) {
	...
	const xp = vE.position.x;
	const zp = vE.position.z;
